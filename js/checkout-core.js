...
export function createOrderPayload({ requestId, form, cart, attribution = null, source = 'GitHub Pages — Точка Хрускоту V3', device = '' }) {
  ...
  return {
    ...
    gift: (form.isGift || secretItems.length) ? { isGift: true, isSurprise: form.signatureMode === 'anonymous' || form.signatureMode === 'hint', hidePrice: true, wrap: '', signatureMode: form.signatureMode || 'named', hint: String(form.hint || '').trim(), courageScenario: '', card: { enabled: Boolean(secretItems.length), type: secretItems.length ? 'universal-secret-key' : '', postcardRequested: Boolean(form.postcardRequested), postcardText: String(form.postcardText || '').trim() } } : { isGift: false },
